...
- **岗位进展评分卡片**
  - 核心指标：**进展分**，计算公式：
    ```
    进展分 = (近7日新增 + SEEK + Contacted × 10) × 肖像分 / 10
    ```
  - **近7日新增**：最近 7 天新接触的候选人数量
  - **SEEK**：达到 SEEK 阶段（强匹配）的候选人数量
  - **Contacted**：已获取联系方式的候选人数量（权重 x10）
  - **肖像分**：基于岗位画像质量的评分（0-10分）
  - 该分数反映了招聘进度的健康程度，结合了量（活跃度）和质（画像准确性、转化率）。
...
- **自动处理按钮**（AI 自动化核心功能）
  - 点击"▶ 自动处理"按钮后，**AI 系统会自动处理列表中的候选人**，实现真正的半自动化招聘：
    ...
    6. **智能 PASS 功能**：
       ...
       - 候选人卡片自动移除（带淡出动画效果）
  - **循环处理模式（Cycle Mode）**：
    - 勾选"循环处理"复选框后，系统会自动在四个标签页（推荐牛人 -> 新招呼 -> 沟通中 -> 牛人已读未回）之间循环切换并处理
    - 未勾选时，系统会持续处理当前模式的候选人，处理完一批后自动刷新列表继续处理，直到无候选人为止
    - 适合无人值守时挂机运行，持续筛选和跟进候选人
  - **效率提升**：一次点击即可完成数十个甚至上百个候选人的筛选和初步沟通，支持长时间自动运行
...
1. **收集反馈**：在候选人详情页点击 **“评分不准”**，填写目标分数与理由（写入 `CN_job_optimizations`）。
2. **管理清单**：在岗位页点击绿色 **“优化肖像”** 入口进入 `/jobs/optimize`。在此处可以查看所有未处理的反馈，**支持删除**无效的反馈项。
3. **生成新版**：勾选若干反馈 → 进入生成页面。系统会基于反馈内容生成新的岗位肖像建议。
   - **防重复机制**：系统会自动拦截已归档（已消费）的反馈项，防止重复优化。
   - **拆分编辑**：新版页面支持 **Keywords（加分项/减分项）** 的独立对比与编辑。
4. **发布闭环**：确认提交后发布新版本。
   - 系统会自动创建新版本的岗位（如 `v5`）。
   - 将本次使用的反馈标记为 `closed`，并记录它们贡献给了哪个版本（`closed_at_job_id`），实现完整的追溯闭环。
