...
from agent.states import Candidate, ManagerState
...
@tool
def get_candidates_tool(mode: str, job_title: str, limit: int) -> str:
    """
...
@tool
# def invoke_recruiter_tool(candidate: dict, mode: str, job_title: str, assistant_name: str, state: Annotated[ManagerState, InjectedState]) -> str:
def invoke_recruiter_tool(
        candidate: dict, mode: str, job_title: str, assistant_name: str, 
        config: RunnableConfig = None, 
        state: Annotated[ManagerState, InjectedState] = None
    ) -> str:
    """
    ...
    Args:
        ...
        mode: str - The mode to process the candidate(required)
        job_title: str - The job position（职位） to fetch the job description(required)
    ...
    system_message = SystemMessage(content=f'现在我们来处理候选人{candidate['name']}，我们使用的助手风格信息如下：\n{assistant_info}')
    human_message = HumanMessage(content=f"这是我的描述，请查收：\n {candidate}")
    ai_message = AIMessage(content=f"你好，我们的岗位描述如下: {job_info}")
    messages = [system_message, human_message, ai_message]

    result = recruiter_graph.invoke(input={
        'mode': mode,
        'candidate': candidate,
        'job_info': job_info,
...
@tool
def send_chat_message_tool(chat_id: str, message: str) -> str:
    """
...
@tool
def get_chat_messages_tool(chat_id: str) -> str:
    """
...
@tool
def request_full_resume_tool(chat_id: str) -> str:
    """
...
@tool
def view_online_resume_tool(chat_id: Optional[str], index:Optional[int], mode: str) -> str:
    """
...
@tool
def view_full_resume_tool(chat_id: str, mode:str) -> str:
    """
...
@tool
def accept_resume_tool(chat_id: str) -> str:
    """
...
@tool
def greet_candidate_tool(identifier: str, message: str, mode: str) -> str:
    """
...
@tool
def request_contact_tool(chat_id: str, message: str) -> str:
    """
