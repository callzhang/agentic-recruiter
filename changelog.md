...
## v2.7.1 (2025-12-31) - 岗位肖像对比体验优化

### ✨ 交互体验提升

#### 岗位肖像对比流程优化
- **岗位列表页**
  - **入口强化**：移除"直接粘贴"按钮，将"对比导入"按钮升级为主要操作入口，引导用户使用更安全的对比更新流程
  - **视觉分层**：通过按钮颜色区分操作优先级，避免误操作覆盖数据

- **优化生成页**
  - **导航优化**：返回按钮文案改为"返回岗位肖像"，且直接跳转至对应岗位的 Tab 页，路径更清晰
  - **视觉引导**："粘贴 JSON" 按钮采用绿色主题，并增加呼吸动画效果（Detect user intent），引导用户进行下一步操作；交互后自动停止动画
  - **按钮增强**：为"拷贝文档"（📄）和"确认提交"（🚀）添加语义化图标，提升界面易读性与操作确认感
