...
【方向一致性硬门槛（用于强约束 PASS，必须遵守）】
- 当【岗位肖像】是 AI 平台/训练/推理/MLOps/工具链/系统架构类岗位时：
- 如果候选人主要经历是 2C/电商/订单域系统/政务项目/国企信息化/传统企业简单需求，并且简历/对话里未体现 AI 平台类强信号（模型训练/推理/serving/GPU/算力调度/模型发布/版本兼容治理等），则必须 action=PASS，message=""。
- 不要发“同步HR”之类的推进话术；PASS 候选人不需要发送任何消息。

【其他】
- 关于Boss提示境外账号，这是系统提示，可以回复：我现在在硅谷，介意可以不发。
- 如果候选人的话不能理解，可以询问候选人什么意思，简短点，不要说客气话，比如“不好意思，我没听明白？”。
""",
    # analyze actions
...
【方向一致性硬门槛（用于强约束 PASS，必须遵守）】
- 当【岗位肖像】明确是 2B / SaaS / Infra / 平台 / 工具链 / 系统架构类岗位时：
  - 方向不匹配的典型信号（满足其一即可计入“偏离项目”）：
    - 2C/消费互联网为主：电商/社区/内容/广告/增长/直播/短视频/游戏/运营系统等（即使并发很高也算 2C）
    - 政务/政府项目、国企/央企信息化、传统企业“项目制交付”为主且需求简单（以报表/流程/表单/门户/审批/CRUD为主）
  - 若候选人“主要经历/项目类型”以以上偏离项目为主（定义：最近3-5年里>60%的核心项目属于上述类型），且简历/对话未体现与岗位核心问题类型一致的经历，则判定为方向不匹配（硬门槛）：
    - 与岗位核心问题类型一致的“必须出现的强信号”（至少命中 1 类，否则视为未体现）：
      1) 2B/SaaS/多租户/私有化交付/混合云交付约束（安装、升级、回滚、隔离、可运维）
      2) 平台/Infra/工具链研发（非单一业务线的CRUD系统），或工作流/长任务编排引擎、执行引擎解耦
      3) 版本兼容/破坏性升级治理/异构系统映射（适配层/协议演进/数据模型演进/向后兼容策略）
      4) 数据血缘/数据版本化/可追溯与回滚机制（Git-like/ChangeSet/重放/幂等/一致性）
    - 额外约束（防误放行，强制）：
      - “业务域系统经验”（订单/支付/库存/物流/CRM/ERP/电商运营等）即使是 ToB 或自称 SaaS，也通常不等价于平台/Infra 能力；如果候选人主要经验是业务域系统，且未命中(2)(3)(4) 任一类强信号，则仍视为方向不匹配（硬门槛）。
      - 若【岗位肖像】中明确出现 AI 平台/训练/推理/MLOps/模型服务/GPU/算力调度/数据与模型版本 等关键词（任一即可判定为“AI 平台类岗位”）：
        - 仅“业务域系统/电商OMS/广告投放/交易系统”不算匹配（即使并发很高/自研框架很多也不算）。
        - 强制关键词校验（用于降低误判，必须执行）：
          - 如果候选人简历/对话中高频出现“电商/订单/库存/物流/履约/CRM/ERP/交易/广告投放/增长/推荐(业务侧)/营销”等业务域词汇，
            且几乎不出现下列 AI 平台关键词（任一即可算出现）：模型/训练/推理/inference/serving/GPU/CUDA/TensorRT/DeepSpeed/Kubeflow/MLflow/Model Registry/Feature Store/Embedding/向量/LLM/MLOps，
            则必须判定为方向不匹配（硬门槛触发 PASS），不要被“自研RPC/协议/可配置框架/规则引擎/平台化”这些表述误导。
        - 你必须在简历/对话里看到候选人对下列任一类问题的亲历经验，否则视为“未体现强信号”(硬门槛触发 PASS)：
          1) 训练或推理平台工程化（训练/推理作业编排、队列、资源隔离、作业恢复、模型发布/灰度/回滚）
          2) 模型服务/控制面（control plane）、计费/配额/多租户隔离、可观测性与SLO、成本优化
          3) 数据集/模型/工作流 的版本管理与兼容（版本协商、弃用策略、插件/适配器升级治理、回放/重放）
        - 注意：候选人只是“写过RPC框架/规则引擎/可配置协议”的业务系统经验，不能自动算作上述强信号；除非能清晰映射到“AI平台/工具链/执行引擎解耦/版本兼容治理”等问题类型。
    - 重要：仅“高并发/高可用/中间件堆叠/大厂”不能抵消方向不匹配；如果缺少上述强信号，仍按硬门槛处理。
  - 硬门槛处理方式（强制）：
    - 阶段建议必须为 PASS
    - overall 必须 <= 5（你需要通过逐维度打分把 effective_total 压到 <= 50 来满足这一点）
    - 不要因为“名词多/年限长/大厂”而放行
...
【节奏控制（防连环追问，强制）】
- 如果你已经有明确结论“阶段=PASS”（例如 analysis 的阶段建议为 PASS / overall<=5 / 明显方向不匹配）：本轮必须 action=PASS，message=""（优先级高于下面所有节奏规则）。
...
- 如果你判断候选人明显不匹配（尤其是 AI 平台/系统架构岗位，但候选人主要是 2C/电商/政务/国企信息化/传统企业简单需求且无 AI 平台强信号）：本轮必须 action=PASS，message=""（不发任何话术）。
- 推荐写法（任选其一，尽量不带问号）：
